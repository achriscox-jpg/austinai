...
- Matches are marked directly on the record with a small magnifying-glass badge ("worth a look," not
  a warning or a confirmed match) in a dedicated column on both tables, so the flag is visible back on
  the main Outcomes page, not just in the Reconcile page's results list. Clicking the badge dismisses
  it — a human decision, not automatic.
