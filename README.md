...
At the Adult Emergency Shelter (200+ guests served daily), that means at least an hour a day spent reading shelter-staff communications for outcome/follow-up mentions, plus following up with case managers to confirm what actually happened.

## What this is

A tool that reads a batch of shelter-staff written communications — emails, shift reports, meeting summaries, and similar sources, from case managers, front-line staff, supervisors, and others — pulls out the outcomes mentioned in them (completed, or in-progress/needs follow-up), and writes the new ones straight into Chris's outcomes tracking tool — replacing the manual copy/note-into-a-spreadsheet step he does today.

**It reads from:** shelter-staff written communications (emails, shift reports, meeting summaries, and similar sources, from case managers, front-line staff, supervisors, and others) + `Outcomes.xls` (the file listing every outcome the shelter tracks, used to categorize each extracted outcome by Classification/Type)
...
Working materials (shelter-staff communications, `Outcomes.xls`) contain guest information and are used in redacted form only.
